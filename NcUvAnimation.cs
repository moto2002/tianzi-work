...
public class NcUvAnimation : NcEffectAniBehaviour
{
	...
	private void Start()
	{
		...
		else
		{
			...
			zero.x = this.m_fTilingX;
			zero.y = this.m_fTilingY;
			base.renderer.material.mainTextureScale = zero;
	...
	private void Update()
	{
		if (this.m_Renderer == null || this.m_Renderer.sharedMaterial == null || this.m_Renderer.sharedMaterial.mainTexture == null || this.m_Timer == null)
		{
			return;
		}
		Vector2 zero = Vector2.zero;
		...
		zero.x = this.m_fOffsetX;
		zero.y = this.m_fOffsetY;
		this.m_Renderer.material.mainTextureOffset = zero;
		if (flag)
		{
			base.OnEndAnimation();
