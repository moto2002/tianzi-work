...
public class UIPopupList : UIWidgetContainer
{
	...
	private void Highlight(UILabel lbl, bool instant)
	{
		if (this.mHighlight != null)
		{
			...
			Vector3 vector = lbl.cachedTransform.localPosition + new Vector3((!this.isCenter) ? (-num) : 0f, y, 1f);
	...
	private void OnClick()
	{
		if (base.enabled && NGUITools.GetActive(base.gameObject) && this.mChild == null && this.atlas != null && this.isValid && this.items.Count > 0)
		{
			...
			this.mHighlight.pivot = ((!this.isCenter) ? UIWidget.Pivot.TopLeft : UIWidget.Pivot.Top);
			...
			while (i < count)
			{
				...
				if (this.isCenter)
				{
					...
					uILabel.cachedTransform.localPosition = new Vector3(this.mBackground.localSize.x + this.padding.x, num5, -1f);
				}
				else
				{
			...
			Vector3 center = new Vector3((!this.isCenter) ? (num7 * 0.5f) : 0f, -num2 * 0.5f, 0f);
