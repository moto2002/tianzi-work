...
public class UIScrollView : MonoBehaviour
{
	...
	public Action OnTopEdge;

	public Action OnBottomEdge;

	public Action OnLeftEdge;

	public Action OnRightEdge;
	...
	public bool RestrictWithinBounds(bool instant, bool horizontal, bool vertical)
	{
		...
		if (vector.sqrMagnitude > 1f)
		{
			...
			if (vector.x > 0f)
			{
				if (this.OnRightEdge != null)
				{
					this.OnRightEdge();
				}
			}
			else if (vector.x < 0f && this.OnLeftEdge != null)
			{
				this.OnLeftEdge();
			}
			if (vector.y > 0f)
			{
				if (this.OnTopEdge != null)
				{
					this.OnTopEdge();
				}
			}
			else if (vector.y < 0f && this.OnBottomEdge != null)
			{
				this.OnBottomEdge();
			}
			return true;
