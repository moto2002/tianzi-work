...
public class UITooltip : MonoBehaviour
{
	...
	protected virtual void SetText(string tooltipText)
	{
		if (this.text != null && !string.IsNullOrEmpty(tooltipText))
		{
			...
			if (this.background != null && !this.background.isAnchored)
			{
			...
			if (this.uiCamera != null)
			{
				this.mPos.x = Mathf.Clamp01(this.mPos.x / (float)ResolutionConstrain.Instance.width);
				this.mPos.y = Mathf.Clamp01(this.mPos.y / (float)ResolutionConstrain.Instance.height);
				float num = this.uiCamera.orthographicSize / this.mTrans.parent.lossyScale.y;
				float num2 = (float)ResolutionConstrain.Instance.height * 0.5f / num;
				Vector2 zero = Vector2.zero;
				zero.x = num2 * this.mSize.x / (float)ResolutionConstrain.Instance.width;
				zero.y = num2 * this.mSize.y / (float)ResolutionConstrain.Instance.height;
				this.mPos.x = Mathf.Min(this.mPos.x, 1f - zero.x);
				this.mPos.y = Mathf.Max(this.mPos.y, zero.y);
			...
			{
				if (this.mPos.x + this.mSize.x > (float)ResolutionConstrain.Instance.width)
				{
					this.mPos.x = (float)ResolutionConstrain.Instance.width - this.mSize.x;
				}
				if (this.mPos.y - this.mSize.y < 0f)
				{
					this.mPos.y = this.mSize.y;
				}
				this.mPos.x = this.mPos.x - (float)ResolutionConstrain.Instance.width * 0.5f;
				this.mPos.y = this.mPos.y - (float)ResolutionConstrain.Instance.height * 0.5f;
			}
		}
		else
		{
