...
public static class NGUIText
{
	...
	public static bool ParseSymbol(string text, ref int index, BetterList<Color> colors, bool premultiply, ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike)
	{
		...
		if (text[index + 4] == ']')
		{
			...
			if (text3 != null)
			{
				...
				{
					NGUIText.<>f__switch$map4 = new Dictionary<string, int>(2)
					{
						{
							"[sub]",
							0
				...
				int num;
				if (NGUIText.<>f__switch$map4.TryGetValue(text3, out num))
				{
					if (num == 0)
					{
						sub = 1;
	...
	public static bool CustomWrapText(string text, out string finalText)
	{
		if (NGUIText.rectWidth < 1 || NGUIText.rectHeight < 1)
		{
		...
		float num = (float)((NGUIText.maxLines <= 0) ? NGUIText.rectHeight : Mathf.Min(NGUIText.rectHeight, NGUIText.fontSize * NGUIText.maxLines));
		float num2 = (float)NGUIText.fontSize + NGUIText.spacingY;
		int num3 = (NGUIText.maxLines <= 0) ? 1000000 : NGUIText.maxLines;
		num3 = Mathf.FloorToInt((num2 <= 0f) ? 0f : Mathf.Min((float)num3, num / num2));
		if (num3 == 0)
		{
			finalText = string.Empty;
			return false;
		...
		int length = text.Length;
		float num4 = (float)NGUIText.rectWidth;
		int num5 = 0;
		int i = 0;
		int num6 = 1;
		int num7 = 0;
		...
		while (i < length)
		{
			...
			if (c == '\n')
			{
				...
				num4 = (float)NGUIText.rectHeight;
				if (num5 < i)
				{
			...
			else
			{
				...
				else
				{
					...
					if (bMSymbol == null)
					{
						float glyphWidth = NGUIText.GetGlyphWidth((int)c, num7);
						if (glyphWidth == 0f)
						{
							goto IL_318;
						}
						num8 = NGUIText.spacingX + glyphWidth;
					}
					else
					{
						num8 = NGUIText.spacingX + (float)bMSymbol.advance;
					}
					num4 -= num8;
					if (num4 < 0f)
					{
						if (flag || num6 == num3)
						{
							...
							{
								num5 = i + 1;
								num4 = (float)NGUIText.rectWidth;
							}
							else
							{
								num5 = i;
								num4 = (float)NGUIText.rectWidth - num8;
							}
							num7 = 0;
						}
						else
		...
		return i == length || num6 <= Mathf.Min(NGUIText.maxLines, num3);
	}

	public static bool WrapSymbolText(string text, out string finalText, int symbolSize)
	{
		if (NGUIText.rectWidth < 1 || NGUIText.rectHeight < 1)
		{
			finalText = string.Empty;
		...
		while (i < length)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						num8 = NGUIText.spacingX + (float)(bMSymbol.advance * (symbolSize / NGUIText.bitmapFont.defaultSize));
					}
					num4 -= num8;
					if (num4 < 0f)
					{
			...
			else
			{
				...
				else
				{
					...
					if (glyphInfo != null)
					{
						...
						if (num8 != 0)
						{
							NGUIText.GlyphInfo expr_489_cp_0 = glyphInfo;
							expr_489_cp_0.v0.x = expr_489_cp_0.v0.x * 0.75f;
							NGUIText.GlyphInfo expr_4A1_cp_0 = glyphInfo;
							expr_4A1_cp_0.v0.y = expr_4A1_cp_0.v0.y * 0.75f;
							NGUIText.GlyphInfo expr_4B9_cp_0 = glyphInfo;
							expr_4B9_cp_0.v1.x = expr_4B9_cp_0.v1.x * 0.75f;
							NGUIText.GlyphInfo expr_4D1_cp_0 = glyphInfo;
							expr_4D1_cp_0.v1.y = expr_4D1_cp_0.v1.y * 0.75f;
							if (num8 == 1)
							{
								NGUIText.GlyphInfo expr_4F1_cp_0 = glyphInfo;
								expr_4F1_cp_0.v0.y = expr_4F1_cp_0.v0.y - NGUIText.fontScale * (float)NGUIText.fontSize * 0.4f;
								NGUIText.GlyphInfo expr_516_cp_0 = glyphInfo;
								expr_516_cp_0.v1.y = expr_516_cp_0.v1.y - NGUIText.fontScale * (float)NGUIText.fontSize * 0.4f;
							}
							else
							{
								NGUIText.GlyphInfo expr_540_cp_0 = glyphInfo;
								expr_540_cp_0.v0.y = expr_540_cp_0.v0.y + NGUIText.fontScale * (float)NGUIText.fontSize * 0.05f;
								NGUIText.GlyphInfo expr_565_cp_0 = glyphInfo;
								expr_565_cp_0.v1.y = expr_565_cp_0.v1.y + NGUIText.fontScale * (float)NGUIText.fontSize * 0.05f;
							}
						}
							...
							if (uvs != null)
							{
								...
								while (l < num15)
								{
									if (glyphInfo.rotatedUVs)
									{
										...
										zero2.x = glyphInfo.u1.x;
										zero2.y = glyphInfo.u0.y;
										uvs.Add(zero2);
										uvs.Add(glyphInfo.u1);
										zero2.x = glyphInfo.u0.x;
										zero2.y = glyphInfo.u1.y;
										uvs.Add(zero2);
									}
									else
									{
										...
										zero3.x = glyphInfo.u0.x;
										zero3.y = glyphInfo.u1.y;
										uvs.Add(zero3);
										uvs.Add(glyphInfo.u1);
										zero3.x = glyphInfo.u1.x;
										zero3.y = glyphInfo.u0.y;
										uvs.Add(zero3);
									}
									l++;
								}
