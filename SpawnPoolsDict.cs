...
public class SpawnPoolsDict : IEnumerable, IDictionary<string, SpawnPool>, ICollection<KeyValuePair<string, SpawnPool>>, IEnumerable<KeyValuePair<string, SpawnPool>>
{
	...
	internal void Add(SpawnPool spawnPool)
	{
		if (this.ContainsKey(spawnPool.poolName))
		{
			if (CachePoolManager.bDebug)
			{
				LogSystem.LogWarning(new object[]
				{
					string.Format("A pool with the name '{0}' already exists. This should only happen if a SpawnPool with this name is added to a scene twice.", spawnPool.poolName)
				});
			}
			return;
		}
	...
	internal bool Remove(SpawnPool spawnPool)
	{
		if (!this.ContainsKey(spawnPool.poolName))
		{
			if (CachePoolManager.bDebug)
			{
				LogSystem.LogWarning(new object[]
				{
					string.Format("PoolManager: Unable to remove '{0}'. Pool not in PoolManager", spawnPool.poolName)
				});
			}
			return false;
		}
	...
	{
		string message = "SpawnPools can only be destroyed, not removed and kept alive outside of PoolManager. There are only 2 legal ways to destroy a SpawnPool: DestroyTime the GameObject directly, if you have a reference, or use PoolManager.DestroyTime(string poolName).";
		throw new NotImplementedException(message);
	}
